...
$windowsPackageVersion = ($packageList.response | where {$_.name -eq "windows"}).version
...
$response = Invoke-RestMethod -UseBasicParsing -Uri  "http://192.168.33.10:5601/api/fleet/package_policies" -ContentType application/json -Headers $headers -Method Post -Body $windowsConfigDictJson
